...
router.get('/checktoken', checktoken,async function(req, res, next) {
  try {
...
router.get('/:id', checktoken, async (req, res)=>{
  try {
      ...
      return res.status(200).json({User: result, decode: req.user});
  } catch (error) {
      console.log("Loi lay danh muc bang id: ", error);
      return res.status(500).json({mess: error})
  }
})
//http://localhost:3000/users/resign
router.post('/resign', async(req, res)=>{
  try {
    const body = req.body;
    const result = await userController.resign(body);
...
router.post('/forgot',async(req, res)=>{
  try {
    ...
    if (checkMail.exists) {
      await userController.sendForgotPasswordEmail(emailForgot, `<h1 style="color: red;">${newPass}</h1>`);
