...
router.get('/:id', async(req, res)=>{
  try {
      ...
      const limit = 5;
      const relatedProducts = await productController.getProductsByIdCate(product.categoryId, id, limit);
      //nếu có lượt xem
...
router.post('/addProduct', upload.single('image'), async (req, res)=>{
  try {
      // const body = req.body;
      const {name, price, description, hot, bestSeller, discount, category} = req.body;
